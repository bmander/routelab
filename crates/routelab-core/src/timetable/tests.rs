...
fn best_by_brute_force(
    ...
    timetable
        ...
        .filter_map(|c| best_by_brute_force(timetable, c.to, c.arrives, to, left - 1))
        .min()
}
...
fn both_models_find_the_connection_worth_making() {
    ...
    let by_stops = earliest_arrival(&table, 0, 28_800, 2, Transfer::instant()).unwrap();
...
fn a_stop_you_cannot_reach_has_no_itinerary() {
    ...
    assert!(earliest_arrival(&table, 2, 0, 0, Transfer::instant()).is_none());
}
...
fn arriving_after_the_last_departure_finds_nothing() {
    ...
    assert!(earliest_arrival(&table, 0, 60_000, 2, Transfer::instant()).is_none());
}
        ...
        for from in 0..12u32 {
            for to in 0..12u32 {
                for at in [0, 900, 1800, 3600] {
                    ...
                    let by_stops = earliest_arrival(&table, from, at, to, Transfer::instant());
                    assert_eq!(
        ...
        for from in 0..10u32 {
            for to in 0..10u32 {
                for query in [
                    ...
                    earliest_arrival(&table, from, 0, to, Transfer::instant()),
                ] {
...
fn you_are_already_where_you_already_are() {
    ...
    for at in [0, 28_800, 60_000] {
        ...
        assert_eq!(
            earliest_arrival(&table, 1, at, 1, Transfer::instant()),
            standing_still
        );
    }
}
