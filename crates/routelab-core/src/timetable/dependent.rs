...
//!
//! ## Why a minimum change time is refused here
//!
//! It is not correct with one label per stop. Staying in your seat is not a
...
//! stop label carries. Charging it always would penalise riding through; never
//! charging it is the simple model.
//!
//! The paper's answer (§4.2) is more nodes: a node per route at each stop, with
//! transfer edges between them through the stop. That is a real construction and
//! it belongs in its own increment. Until then this refuses a non-zero
//! [`Transfer`] rather than approximating one, so the two models stay exactly
//! comparable on the ground they share.
...
/// # Panics
///
/// If `transfer` is not [`Transfer::instant`] — see the module docs.
pub fn time_dependent_query(
    timetable: &Timetable,
    from: NodeId,
    at: Time,
    to: NodeId,
    ...
    assert_eq!(
        transfer.minimum, 0,
        "a minimum change time needs a node per route; see the module docs"
    );
    ...
    while let Some(Reverse((now, stop))) = queue.pop() {
        ...
        for (next, connections) in timetable.edges_from(stop) {
            // The one line the static case does not have: not "what does this
            // edge cost" but "what is the next thing along it".
            let boarding = connections.partition_point(|c| c.departs < now);
            let Some(connection) = connections[boarding..]
                .iter()
                .min_by_key(|c| (c.arrives, c.departs))
            else {
                continue;
            };
            if connection.arrives < earliest[next as usize] {
                earliest[next as usize] = connection.arrives;
                arrived_by[next as usize] = Some(Ride::from(*connection));
