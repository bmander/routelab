...
Three rows, one per technique, and the numbers that matter: what binding cost,
...
The point of running them side by side is that they are not three speeds of
...
Dijkstra; RAPTOR builds no graph at all and scans routes in rounds. The
settled column is where that shows — read it against the `nodes` column,
because they do not count the same thing.
